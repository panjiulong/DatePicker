...
protocol DatePickerViewDataSource: class {
    func datepicker(_ picker: DatePickerView, numberOfRowsInComponent component: Int) -> Int
    func datepicker(_ picker: DatePickerView, intValueForRow row: Int, forComponent component: Int) -> Int
}
...
class DatePickerView: UIView {
    ...
    var yearAttributedString:((String)->NSAttributedString)?
    var monthAttributedString:((String)->NSAttributedString)?
    var dayAttributedString:((String)->NSAttributedString)?
    ...
    var currentDate: Date? {
        ...
        guard let date = Date.jl_defaultFormatter.date(from: "\(year)-\(month)-\(day)") else{ return nil}
        return date
    }
    ...
    fileprivate var manager: JLDateManager?
    ...
    required init?(frame: CGRect = .zero, minDate: Date, maxDate: Date) {
        ...
        manager = JLDateManager.init(minDate: minDate, maxDate: maxDate)
...
extension DatePickerView{
    ...
    func setDate(_ dateString: String, animated: Bool = false) {
        guard let date = Date.jl_defaultFormatter.date(from: dateString) else { return }
...
extension DatePickerView: JLDateSelectionDelegate {
...
extension DatePickerView: UIPickerViewDelegate{
    ...
    func pickerView(_ pickerView: UIPickerView, attributedTitleForRow row: Int, forComponent component: Int) -> NSAttributedString? {
        ...
        switch component {
        case 0:
            return yearAttributedString?(String(intValue))
        case 1:
            return monthAttributedString?(String(intValue))
        default:
             return dayAttributedString?(String(intValue))
        }
...
protocol JLDateSelectionDelegate: class {
    func currentYearInt() -> Int
    func currentMonthInt() -> Int
    func currentDayInt() -> Int
}
...
class JLDateManager {
    ...
    weak var delegate: JLDateSelectionDelegate?
...
extension JLDateManager {
    ...
    func setDate(_ date: Date) -> (yRow: Int, mRow: Int, dRow: Int)? {
        if date.compare(minDate) == .orderedAscending || date.compare(maxDate) == .orderedDescending {
            NSLog("指定日期超过了可选范围")
            let result = refreshCurrent(year: minDate.year, month: minDate.month, day: minDate.day)
            return result
        }
...
extension JLDateManager: DatePickerViewDataSource {
    func datepicker(_ picker: DatePickerView, numberOfRowsInComponent component: Int) -> Int {
...
extension Date{
    static var jl_defaultFormatter: DateFormatter {
